...
Roles can be overridden per project (e.g., a Submitter on most projects who's a Tech on one). Project membership also carries an **Agent** flag — orthogonal to role. The Agent flag controls who appears in the assignment pool for auto-assignment policies + escalation targets on that specific project.

---

## Notes tab (handler-only)

Since: v0.7.0. Every ticket carries a **Notes** tab visible only to Admin / Manager / Tech. Notes are project-handler-only commentary, separate from comments:
