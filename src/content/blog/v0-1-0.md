...
The Notify Vendor button gates outbound. Resolvd never auto-blasts a vendor on ticket creation — staff review the ticket, click Notify Vendor, and only then do status-change and resolved updates fire automatically. After the first outbound, vendor replies thread back into the same ticket via the inbound parser.

Each vendor reply gets a per-vendor color pill — same vendor, same color across users and sessions, hashed deterministically from the company id.

**How to use:** Admin → Companies → New. Add contacts, attach to tickets, click Notify Vendor when you're ready.
...
[v0.1.0 on GitHub](https://github.com/joshhearne/resolvd/releases/tag/v0.1.0) has the complete commit list. This post covers the headline features; smaller wins (bulk edit, ticket merge, NATO phonetic readback, vendor pill colors, browser push, markdown comments) live in their own follow-up posts.
