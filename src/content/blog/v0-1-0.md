...
![Admin → Email backends, with a connected Microsoft 365 mailbox showing the Monitor inbox toggle, vendor-emails-as-submitting-user toggle, and the inbound banner stripping / project scope panels.](/blog/v0-1-0/email-to-ticket.png)
...
![Admin → Companies, project-scoped vendor list — Stratus Systems Inc., Lumen Electric Co., BlueRidge Mechanical — each with domain, project, and active contact count.](/blog/v0-1-0/vendor-crm.png)
...
![Admin → Encryption settings page describing the three modes (off / standard / vault) and the runbook to enable Standard mode.](/blog/v0-1-0/encryption-at-rest.png)
...
![Admin → Support access, Active grants tab. One ACTIVE grant for Iris Doyle (Support) with the request reason, approver, and expiry timestamp; Revoke now action on the right.](/blog/v0-1-0/jit-support-access.png)
...
![Admin → Authentication settings — Microsoft Entra ID and Local username/password enabled side by side, with multi-factor authentication toggles per role and the outbound email backend selector.](/blog/v0-1-0/auth-providers.png)
