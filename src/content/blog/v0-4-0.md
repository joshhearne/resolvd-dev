...
![Admin → Email backends with the Project scope panel expanded — HearneTech (HT) scoped with both send + recv enabled, an approved badge, and the Add-project picker for adding more scopes.](/blog/v0-4-0/helpdesk-routing.png)
...
![Admin → Merge tickets dialog. Two slots A/B; slot B with "STRAT" typed shows live results from the Stratus Cloud Migration project — title, description preview, and external status — ranked by match.](/blog/v0-4-0/ticket-merge.png)
...
![Inbound banner stripping panel — preset buttons for Inky, Mimecast, Proofpoint, Avanan above a multi-line regex textarea, with the gateway-first guidance highlighted.](/blog/v0-4-0/banner-stripping.png)
...
![Send-As modal on a vendor-visible comment — Send as me (Shotbot) above Send as submitter (Priya Iyer). The acting human picks the display-name identity while From stays the connected mailbox.](/blog/v0-4-0/vendor-outbound.png)
...
![STRAT-0001 thread with a vendor reply from Calvin Ortiz tagged FROM STRATUS SYSTEMS INC. in the deterministic per-vendor color, contrasting the brand-green TO VENDOR pill on staff comments above.](/blog/v0-4-0/per-vendor-pill.png)
...
![Admin → Branding, Cross-project visibility section. Org-wide defaults for restricting @mentions and the follower picker to project members, sitting above the localization (date/time/timezone) controls.](/blog/v0-4-0/cross-project-visibility.png)
